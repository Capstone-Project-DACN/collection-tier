...
const CRON_TAG = 'CRON_JOB_CONSUMER_ANOMALY'
const CRON_TIME = process.env.CRON_TIME_CONSUMER_ANOMALY
const IS_ENABLED = +process.env.CRON_CONSUMER_ANOMALY_ENABLED
...
const handle = async (batch_size, current_slot) => {
    ...
    try {
        ...
        const data = await httpConsumerPolling.pollAnomalyData(batch_size)
        await dataTransferService.transferAnomalyData(data)
