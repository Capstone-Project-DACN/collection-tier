...
const CRON_TAG = 'CRON_JOB_CONSUMER_HOUSEHOLD'
const CRON_TIME = process.env.CRON_TIME_CONSUMER_HOUSEHOLD
const IS_ENABLED = +process.env.CRON_CONSUMER_HOUSEHOLD_ENABLED
...
const handle = async (batch_size, current_slot) => {
    ...
    try {
        ...
        const data = await httpConsumerPolling.pollHouseholdData(batch_size)
        await dataTransferService.transferHouseholdData(data)
